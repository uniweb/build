...
describe('a route-bound entry belongs to its own page, in either mode (2026-09-11)', () => {
  // A page's own data step asks for the views its route binds — its branch's, a `deferred:`
  // query's per-record file — and each is filed under the page that asked (`loadPageData` in the
  // render loop since 2026-09-20; `readRouteBoundViews` before it). Carried to every page instead,
  // a site of N such pages would embed N views — or N whole records — in each of them (measured on
  // the first version of the bake).
